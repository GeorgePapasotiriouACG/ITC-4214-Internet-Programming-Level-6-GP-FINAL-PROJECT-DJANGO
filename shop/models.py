...
class UserProfile(models.Model):
    ...
    # Base32-encoded secret key generated once per user at 2FA setup.
    # Combined with pyotp, this creates time-based one-time passwords (RFC 6238).
    totp_secret = models.CharField(max_length=64, blank=True, default='')
    totp_enabled = models.BooleanField(default=False)
    # ── Accessibility preferences ─────────────────────────────────────────────
    # Stores the user's saved accessibility settings across sessions.
    # Example: {"text_size": "large", "colorblind": "deuteranopia", "high_contrast": true}
    accessibility_prefs = models.JSONField(default=dict, blank=True)
...
class Product(models.Model):
    ...
    # Short demo video (mp4 / webm) auto-played muted on card hover
    video_demo = models.FileField(upload_to='product_videos/', blank=True, null=True,
                                   help_text="Short demo clip (mp4/webm). Auto-plays muted on hover.")
    ...
    # Comma-separated product IDs from collaborative filtering — updated by build_collab_filter management command
    collab_recs = models.CharField(max_length=200, blank=True, default='',
                                   help_text='Comma-separated product IDs (auto-generated by build_collab_filter)')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        # Performance indexes on the most-filtered columns across the platform.
        # Verified via query analysis: product listings, search, and admin pages
        # all hit these columns in WHERE / ORDER BY clauses under high load.
        indexes = [
            models.Index(fields=['is_active', 'is_approved', 'category']),
            models.Index(fields=['is_active', 'is_approved', 'price']),
            models.Index(fields=['is_active', 'is_approved', '-views_count']),
...
class ViewedProduct(models.Model):
    ...
    class Meta:
        ordering = ['-viewed_at']
        # Index allows fast lookup of "what did user X view recently?" (recommender)
...
class Order(models.Model):
    ...
    class Meta:
        ordering = ['-created_at']
        # Index for "my orders" page: filter by user, ordered by date (most common query)
...
# ─── FAQ ──────────────────────────────────────────────────────────────────────
# Frequently Asked Questions displayed on the FAQ page and fed to the AI
# assistant as platform knowledge. Grouped by category for easy navigation.

class FAQCategory(models.Model):
    """A grouping label for FAQ entries, e.g. 'Orders', 'Returns', 'Account'."""
    name = models.CharField(max_length=100)
...
class FAQ(models.Model):
    ...
    answer = models.TextField()
    # Higher priority FAQs appear first within a category
    order = models.PositiveIntegerField(default=0)
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        # Composite index for fast lookups in the AI assistant FAQ query
...
# Stores browser push subscription objects (endpoint + keys) so the server
# can push notifications to users even when the browser tab is closed.
# Subscriptions are created by the JS navigator.serviceWorker + PushManager API.

class PushSubscription(models.Model):
    user = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.CASCADE, related_name='push_subscriptions'
    )
    ...
    def __str__(self):
        return f"PushSub({self.user_id or 'anon'}, {self.endpoint[:40]})"


# ─── Database Index additions to existing models (via proxy Meta) ─────────────
# Django's Meta.indexes declared here are generated as separate migrations.
# These cover the most frequently filtered/sorted columns across the platform.
# Each index listed targets a hot-path query identified during the performance audit.

# NOTE: indexes are defined directly on each model's Meta class above where
# possible. Additional composite indexes are added here via model updates.
